...
func _ready() -> void:
	# Keep this first character as the visual base model.
	# Move it beside the upper island, then duplicate the exact same model beside the middle island.
	position = Vector3(-3.5, 0.0, -4.5)
	_build_customer(self)

	var middle_customer := Node3D.new()
	middle_customer.name = "MiddleCustomerPreview"
	middle_customer.position = Vector3(0.0, 0.0, 5.0)
	add_child(middle_customer)
	_build_customer(middle_customer)
