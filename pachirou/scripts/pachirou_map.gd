...
const BOARD_BOTTOM_LEFT := Vector2(-20.0,-46.0)
const BOARD_BOTTOM_RIGHT := Vector2(16.0,-28.0)
const BOARD_THICKNESS := Vector2(3.0,-1.5)
const EQUIPMENT_DEPTH := Vector2(10.0,-5.0)
const EQUIPMENT_FRONT_LEFT := Vector2(-26.0,-39.0)
...
func _create_island_bay(cell: Vector2i) -> void:
	...
	_create_island_backboard(bay)
	_create_machine_and_sand(bay)
	_create_island_shelf_and_counter(bay)
...
func _create_island_backboard(parent: Node2D) -> void:
	var up := Vector2(0,-BACKBOARD_HEIGHT)
	_add_poly(parent,PackedVector2Array([BOARD_BOTTOM_LEFT,BOARD_BOTTOM_RIGHT,BOARD_BOTTOM_RIGHT+up,BOARD_BOTTOM_LEFT+up]),BACKBOARD,1)
	_add_poly(parent,PackedVector2Array([BOARD_BOTTOM_RIGHT,BOARD_BOTTOM_RIGHT+BOARD_THICKNESS,BOARD_BOTTOM_RIGHT+BOARD_THICKNESS+up,BOARD_BOTTOM_RIGHT+up]),BACKBOARD_SIDE,1)

func _create_machine_and_sand(parent: Node2D) -> void:
	var machine_lb := EQUIPMENT_FRONT_LEFT
...
func _create_island_shelf_and_counter(parent: Node2D) -> void:
	var shelf_front_left := BOARD_BOTTOM_LEFT+Vector2(0,-MACHINE_HEIGHT-4)
	var shelf_front_right := BOARD_BOTTOM_RIGHT+Vector2(0,-MACHINE_HEIGHT-4)
	var shelf_depth := Vector2(-8,4)
	var shelf_back_left := shelf_front_left+shelf_depth
	var shelf_back_right := shelf_front_right+shelf_depth
	...
	_add_poly(parent,PackedVector2Array([shelf_front_left,shelf_front_right,shelf_front_right+drop,shelf_front_left+drop]),SHELF_EDGE,20)
	var machine_front_right := EQUIPMENT_FRONT_LEFT+MACHINE_FRONT_VECTOR
	var machine_center := (EQUIPMENT_FRONT_LEFT+machine_front_right)*0.5+Vector2(0,-MACHINE_HEIGHT-5)
	var axis := MACHINE_FRONT_VECTOR.normalized()*7.0
	var counter_lb := machine_center-axis
	var counter_fb := machine_center+axis
...
func _create_stool(cell: Vector2i) -> void:
	...
	stool.name = "Stool"
	var machine_center_x := (EQUIPMENT_FRONT_LEFT.x+(EQUIPMENT_FRONT_LEFT+MACHINE_FRONT_VECTOR).x)*0.5
